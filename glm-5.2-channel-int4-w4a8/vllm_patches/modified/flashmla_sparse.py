...
class FlashMLASparseImpl(MLACommonBaseImpl[FlashMLASparseMetadata]):
    ...
    def __init__(
        ...
        **mla_args,
        ...
        # GLM-5.2: shared 层 indexer=None, 直接用透传的共享 topk_indice_buffer.
        # full 层优先用 indexer.topk_indices_buffer (与原行为一致).
        if indexer is not None:
            self.topk_indices_buffer: torch.Tensor | None = (
                indexer.topk_indices_buffer)
        else:
            self.topk_indices_buffer = topk_indice_buffer
    ...
    def _bf16_flash_mla_kernel(
        ...
        from vllm.v1.attention.backends.mla.sparse_mla_fwd import flash_mla_sparse_fwd
