...
def update_dflash(config_dict: dict, vllm_config: dict) -> None:
    """
    ...
    """
    vllm_config["architectures"] = ["DFlashDraftModel"]
    vllm_config["draft_vocab_size"] = config_dict.get("draft_vocab_size")
    if config_dict.get("target_hidden_size") is not None:
        vllm_config["target_hidden_size"] = config_dict["target_hidden_size"]

    aux_layer_ids = config_dict["aux_hidden_state_layer_ids"]
    vllm_config["eagle_aux_hidden_state_layer_ids"] = aux_layer_ids

    # DFlash configs use different indexing for the target layers, see #40727
    vllm_config["dflash_config"] = {
    ...
    vllm_config["dflash_config"]["causal"] = not config_dict.get(
        "sliding_window_non_causal", True
    )


@register_speculator("dspark")
def update_dspark(config_dict: dict, vllm_config: dict) -> None:
    """
    ...
    - sample_from_anchor: Whether to sample from the anchor position. Default
        ...
        block_size - 1 speculative tokens).
    """
    vllm_config["architectures"] = ["Qwen3DSparkModel"]
    vllm_config["sample_from_anchor"] = config_dict.get("sample_from_anchor", False)

    aux_layer_ids = config_dict["aux_hidden_state_layer_ids"]
    vllm_config["eagle_aux_hidden_state_layer_ids"] = aux_layer_ids
    # DSpark indexes target layers as aux_id - 1 (matches the dense configs).
    vllm_config["target_layer_ids"] = [i - 1 for i in aux_layer_ids]
    ...
    for key in (
        ...
        if config_dict.get(key) is not None:
            vllm_config[key] = config_dict[key]
