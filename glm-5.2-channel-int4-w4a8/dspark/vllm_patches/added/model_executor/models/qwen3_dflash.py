...
class DFlashQwen3Model(nn.Module):
    ...
    # Stacked (q/k/v -> qkv_proj, gate/up -> gate_up_proj) is handled in
    # load_weights via stacked_params_mapping, since the preinstalled vLLM
    # WeightsMapper does not support orig_to_new_stacked / data.shard_id.
    # Backported: the preinstalled QKVParallelLinear.weight_loader_v2 (used when
    # the draft is online-quantized, which creates v2 params) asserts the shard
    # id is one of "q"/"k"/"v" (strings), while MergedColumnParallelLinear's v2
    # loader wants integer indices. Match each loader's expected type so both
    # the unquantized and online-quantized draft load paths succeed.
    ...
    def _build_context_kv_buffers(
        ...
        # Detect the preinstalled vLLM "NN" weight layout (VLLM_USE_NN default
        # True on this ROCm build): linear weights are stored transposed as
        # [input_size, output_size] and the forward does matmul(x, weight).
        # The standard vLLM layout is [output_size, input_size] with F.linear.
        # qkv_proj spans [q_size | k_size | v_size] along the OUTPUT axis, so
        # the KV slice axis differs between the two layouts.
        self._nn_layout = bool(envs.VLLM_USE_NN)
        if self._nn_layout:
            # weight: [hidden, q_size + 2*kv_size] -> KV slice on dim=1
            kv_weights = [a.qkv_proj.weight[:, a.q_size :] for a in layers_attn]
            # -> [hidden, num_layers * 2 * kv_size]
            self._fused_kv_weight = torch.cat(kv_weights, dim=1)
        else:
            # weight: [q_size + 2*kv_size, hidden] -> KV slice on dim=0
            kv_weights = [a.qkv_proj.weight[a.q_size :] for a in layers_attn]
            # -> [num_layers * 2 * kv_size, hidden]
    ...
    def _project_context_kv(
        ...
        if self._nn_layout:
            # NN layout: fused weight is [hidden, num_layers*2*kv_size].
            # matmul([num_ctx, hidden], [hidden, out]) -> [num_ctx, out].
            all_kv_flat = torch.matmul(normed_context_states,
    ...
    def precompute_and_store_context_kv(
        ...
        for i in range(L):
            ...
            # bind_kv_cache wraps each layer's cache as a single-element list
            # (forward_context[layer].kv_cache = [kv_cache]). The unified
            # attention op unwraps it internally, but this hand-rolled cache
            # insert calls impl.do_kv_cache_update directly, which on ROCm does
            # `key_cache, value_cache = kv_cache` and expects the (key, value)
            # tuple, not the wrapper list.
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # Apply non-stacked name remapping (midlayer/encoder renames) first.
        remapped = self.hf_to_vllm_mapper.apply(self._preprocess(weights))
        for name, loaded_weight in remapped:
            ...
            for param_name, weight_name, shard_id in self.stacked_params_mapping:
                ...
                if name_mapped not in params_dict:
                    # Not a stacked param on this model; fall through to direct.
                    continue
