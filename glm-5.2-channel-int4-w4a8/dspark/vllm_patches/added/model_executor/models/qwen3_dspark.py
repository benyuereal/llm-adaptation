...
class DSparkMarkovHead(nn.Module):
    ...
    def __init__(
        ...
        # Backported: the preinstalled ParallelLMHead has no `disable_tp` arg
        # (upstream forces full replication with it). Here the head is consumed
        # only through LogitsProcessor (bias / compute_draft_logits), which
        # gathers the sharded rows and all-reduces, so default TP sharding of
        # markov_w2 is correct and matches how the main lm_head is sharded.
        # The direct-weight path (apply_bias_gathered) is only used when a
        # reduced draft vocab (dspark_draft_topk) is configured, which this
        # full-vocab checkpoint is not.
