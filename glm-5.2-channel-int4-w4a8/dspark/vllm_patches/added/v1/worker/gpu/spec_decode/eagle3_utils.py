...
"""EAGLE3 / DFlash / DSpark auxiliary hidden state layer utilities.

Backported from upstream vLLM. Resolves the target-model layer indices whose
intermediate hidden states feed the drafter, supporting eagle3, dflash and
dspark config semantics.
"""
...
def get_eagle3_aux_layers_from_config(
    ...
    if not layer_ids:
        # DSpark draft configs name these directly and use 0-indexed absolute
        # target layer indices (matching the EAGLE3 convention, no +1 shift).
