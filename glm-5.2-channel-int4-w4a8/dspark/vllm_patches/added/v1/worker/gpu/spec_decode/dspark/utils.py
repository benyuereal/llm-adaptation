...
from vllm.v1.worker.gpu.spec_decode.eagle_utils import (
    _should_share,
    get_target_lm_head,
)
...
def load_dspark_model(target_model: nn.Module, vllm_config: VllmConfig) -> nn.Module:
    ...
    # Backported: the preinstalled SpeculativeConfig has no `attention_backend`
    # / `kv_cache_dtype` fields, so resolve them defensively.
    draft_attention_backend = (
        getattr(speculative_config, "attention_backend", None)
        or vllm_config.attention_config.backend
    )
    draft_kv_cache_dtype = getattr(speculative_config, "kv_cache_dtype", None)
    ...
    draft_vllm_config = replace(
        ...
        cache_config=(
            replace(
                ...
                cache_dtype=draft_kv_cache_dtype,
            )
