...
class MiniMaxSparseAttnBackend(AttentionBackend):
    def __init__(self, runner: "ModelRunner"):
        ...
        # EAGLE3 verify: draft token count per request (D). Fixed Python int,
        # graph-safe. Used in forward_extend's target_verify branch to build
        # cu_seqlens / seq_lens WITHOUT depending on the temporary
        # `forward_batch.extend_seq_lens` tensor (which is None for verify and
        # would be newly allocated each forward_extend call — under cuda graph
        # capture that temp tensor is GC'd after capture, so replay would read
        # a stale/freed address → garbage → VMFault. See forward_extend verify
        # branch for the graph-safe reconstruction using this + forward_batch.seq_lens).
        self.num_draft_tokens = int(
            getattr(runner.server_args, "speculative_num_draft_tokens", None) or 4
        )
    ...
    def forward_extend(
        ...
        *,
        ...
        # EAGLE3 Strategy B: route target_verify through the NEW graph-safe
        # verify_prefill kernel (minimax_sparse_verify_prefill) instead of the
        # crashing prefill path. Strategy A (verify->decode kernel) is abandoned
        # — it broke dialogue output because the decode kernel uses heuristic
        # init_blocks/local_blocks block selection instead of prefill's strict
        # causal mask (off_q >= pos_k). Strategy B preserves prefill's EXACT
        # causal semantics; only the score buffer SIZE changes (fixed upper
        # bound max_seqblock_k_upper = cdiv(context_len, block_size_k)) so
        # capture/replay shapes are identical (graph-safe). The actual kernel
        # call selection (verify_prefill vs original prefill) happens at the
        # bottom of this method; here we just no longer short-circuit to the
        # decode kernel. _forward_verify_via_decode / _verify_decode_mode are
        # kept (unused) for reference / easy rollback.
        ...
        # ---- cu_seqlens / seq_lens / prefix_lens 构建 ----
        # 【graph-safety 关键】EAGLE3 target_verify 进 cuda graph (capture_forward_mode
        # =TARGET_VERIFY). forward_extend 的 Python 在 replay 时被 graph 跳过, 所以
        # 这里构建的 tensor 的 *地址* 会被 graph 记录, replay 时必须仍有效.
        #
        # 旧实现 (已废弃): 用 forward_batch.extend_seq_lens 临时 tensor 算
        #   cu_seqlens = torch.cat([zeros, extend_seq_lens.cumsum(0)])
        #   seq_lens = raw_seq_lens + extend_seq_lens
        # 但 verify 时 ForwardBatch.init_new (forward_batch_info.py:561 is_target_verify
        # 分支) **不设 extend_seq_lens** (保持 None), 旧代码在 forward_extend 里
        # torch.full 临时新建它. 这个临时 tensor 在 capture 的 forward_batch (局部变量,
        # cuda_graph_runner.py:1072) 被 GC 后内存释放/复用, replay 时 graph 读老地址
        # → 垃圾 → 越界 → VMFault (bs=1 碰巧不炸, bs>=并发阈值必崩).
        #
        # 新实现 (graph-safe): verify 完全不依赖 extend_seq_lens, 改用
        #   1. forward_batch.seq_lens  (graph buffer, replay_prepare 填充, 地址稳定)
        #   2. self.num_draft_tokens D (Python int 常量, graph-safe)
        #   3. torch.arange(0, (bs+1)*D, D)  (Python int 输入, 同 triton backend 的
        #      qo_indptr, sglang 标准的 graph-safe 模式)
        # verify 时 forward_batch.seq_lens = prefix (prepare_for_v2_verify 不加 D),
        # 故 prefix_lens=seq_lens, seq_lens=seq_lens+D, cu_seqlens=[0,D,2D,...,bs*D].
        raw_seq_lens = forward_batch.seq_lens.to(torch.int32)
        is_verify = forward_batch.forward_mode.is_target_verify()
        if is_verify:
            D = self.num_draft_tokens
            bs = raw_seq_lens.shape[0]
            device = raw_seq_lens.device
            # cu_seqlens = [0, D, 2D, ..., bs*D]. 输入全是 Python int (bs 是 capture
            # 固定值, D 是常量), 输出内容确定 → graph-safe (同 triton_backend.py:363
            # qo_indptr = torch.arange 的标准模式).
            cu_seqlens = torch.arange(
                0, (bs + 1) * D, step=D, dtype=torch.int32, device=device
            )
            # prefix_lens = forward_batch.seq_lens (graph buffer, 地址稳定).
            # raw_seq_lens 是 .to(int32) no-op (seq_lens 已是 int32), 同地址.
            prefix_lens = raw_seq_lens
            # seq_lens = prefix + D. raw_seq_lens 是 graph buffer, + Python int D
            # 是 graph-safe (输入地址稳定, 输出是 graph 内部静态内存, 内容随 replay
            # 的 raw_seq_lens 值正确更新).
            seq_lens = raw_seq_lens + D
        elif forward_batch.extend_seq_lens is not None:
            # normal extend: extend_seq_lens 由 init_new 设为真实 tensor (eager, 非
            # graph 路径, 地址本次 forward 存活). 用原 prefill 的 cat 逻辑.
            ...
            # fallback: 无 extend_seq_lens (不应发生于 verify/extend, 兜底).
            cu_seqlens = torch.cat(
                [
                    torch.zeros(1, dtype=torch.int32, device=raw_seq_lens.device),
                    raw_seq_lens.cumsum(0).to(torch.int32),
                ]
            )
            prefix_lens = torch.zeros_like(raw_seq_lens)
            seq_lens = raw_seq_lens
        ...
        # EAGLE3 Strategy B: target_verify uses the NEW graph-safe verify_prefill
        # kernel (fixed-size score buffer). Normal extend keeps the original
        # prefill kernel unchanged. The verify kernel takes max_seqblock_k_upper
        # (Python int = cdiv(context_len, block_size_k)) so its score buffer is a
        # FIXED size across capture/replay → graph-safe. The kernel still loops
        # over the real seq_lens for causal masking (no compute waste). For
        # verify, self._max_seqlen_k is already set to self._max_seqlen_k_upper
        # by init_forward_metadata_capture/replay (scheme C) — keep passing it
        # for consistency; it is only a hint (the kernel uses the seq_lens
        # tensor for causal and max_seqblock_k_upper for the buffer size).
        if forward_batch.forward_mode.is_target_verify():
            ...
            if (_VERIFY_PROBE and _VERIFY_PROBE_RANK == 0
                ...
                try:
                    ...
                    _probe_log(f"  idx_k_cache: shape={tuple(idx_k_cache.shape)}")
                    if idx_v_cache is not None:
                        _probe_log(f"  idx_v_cache: shape={tuple(idx_v_cache.shape)}")
                    _probe_log(f"  cu_seqlens: shape={tuple(cu_seqlens.shape)} ptr={cu_seqlens.data_ptr()} "
                               f"dtype={cu_seqlens.dtype}")
                    _probe_log(f"  seq_lens: shape={tuple(seq_lens.shape)} ptr={seq_lens.data_ptr()}")
                    _probe_log(f"  prefix_lens: shape={tuple(prefix_lens.shape)} ptr={prefix_lens.data_ptr()}")
                    _probe_log(f"  req_pool_indices: shape={tuple(forward_batch.req_pool_indices.shape)} "
                               f"ptr={forward_batch.req_pool_indices.data_ptr()}")
