...
def flash_prefill_with_topk_index(
    ...
    max_seqblock_k_upper: Optional[int] = None,
):
    ...
    # Graph-safe score allocation. score's 3rd dim is sized off the K-block
    # count. Under EAGLE3 TARGET_VERIFY this prefill path runs inside a cuda
    # graph: capture sees a dummy seq_lens=1 (tiny max_seqblock_k) while replay
    # sees the real seq_lens (~thousands). cuda graph locks the allocation
    # shape at capture, so replay writes score[..., real_block_id] past the
    # captured 3rd dim -> out-of-bounds -> VMFault. When a constant upper bound
    # is supplied (cdiv(context_len, block_size_k)), allocate against it so the
    # shape is identical at capture and replay. Kernels index score via real
    # cu_seqblocks_k / seq_lens with boundary_check, so the extra slots (filled
    # -inf) are never read; causal semantics are unchanged. max_seqblock_k
    # (from the live max_seqlen_k) is retained above for any downstream use.
    score_max_seqblock_k = (
        max_seqblock_k_upper
        if max_seqblock_k_upper is not None
        else max_seqblock_k
    )
    if disable_index_value:
        o = None
    ...
    score = torch.full(
        (num_heads, total_q, score_max_seqblock_k),
        float("-inf"),
        dtype=torch.float32,
        device=q.device,
    )
