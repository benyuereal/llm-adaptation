...
class MiniMaxM3DecoderLayer(nn.Module):
    ...
    * If present, the per-layer dense/sparse split is read from
    ...
    def __init__(
        ...
        # Determine if this layer is MoE (sparse) or dense FIRST,
        # since dense layers (0-2) have unquantized weights in checkpoint
        moe_layer_freq = getattr(config, "moe_layer_freq", None)
        mlp_layer_types = getattr(config, "mlp_layer_types", None)
        if moe_layer_freq is not None:
            self.is_layer_sparse = moe_layer_freq[layer_id] != 0
        elif mlp_layer_types is not None:
            self.is_layer_sparse = mlp_layer_types[layer_id] != "dense"
        else:
            self.is_layer_sparse = True

        # Dense layers have unquantized weights in checkpoint
        layer_quant_config = quant_config if self.is_layer_sparse else None

        # Attention: dense layers use BF16, MoE layers use INT4 quantization
        attn_quant_config = layer_quant_config
        ...
        def _is_layer_sparse(lid):
            ...
            if mlp_layer_types is not None:
                if lid < 0 or lid >= config.num_hidden_layers:
                    return True
                return mlp_layer_types[lid] != "dense"
            return True
